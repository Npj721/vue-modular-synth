...
| **WavetableS** | Table de wavetables parcourue par morphing (2 oscillateurs + crossfade equal-power), **en boucle** tant que la note est tenue ; release en fin de crossfade | `frequency` (param), `detune` (param) | `out` (audio) | `detune`, `delay`, `morph` en ms (durée d'un crossfade entre 2 frames, défaut 200 ms), `endMode` (défaut `loop` = retour à la frame 0 en fin de liste, ou `pingpong` = va-et-vient), `wave` (JSON `[ { real, imag }, … ]` jusqu'à 255 frames) |
